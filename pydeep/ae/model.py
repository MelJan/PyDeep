...
class AutoEncoder(BipartiteGraph):
    ...
    def _get_sparse_penalty(self, h, factor, desired_sparseness):
        ...
        if self.hidden_activation_function == Sigmoid:
            ...
            sparseness = desired_sparseness*numx.log(desired_sparseness/mean_h)+(1.0
                         -desired_sparseness)*numx.log((1.0-desired_sparseness)/(1.0-mean_h))
        else:
    ...
    def _get_contractive_penalty_gradient(self, x, a_h, df_a_h):
        ...
        grad_w = numx.dot((x-self.ov).T,2.0*df_a_h*ddf_a_h)*w2_sum/x.shape[0] + 2.0*self.w*(numx.mean(df_a_h**2.0,axis = 0))
        grad_c = numx.mean(grad_c,axis =0).reshape(1,self.output_dim)
    ...
    def _get_slowness_penalty_gradient(self, x, x_next, h, h_next, df_a_h, df_a_h_next):
        ...
        diff = 2.0*(h-h_next)
        grad_w = (numx.dot((x-self.ov).T,diff*df_a_h)-numx.dot((x_next-self.ov).T,diff*df_a_h_next))/x.shape[0]
        grad_c = numx.mean(diff*(df_a_h-df_a_h_next),axis=0)
        return [grad_w, grad_c]

    def finit_differences(self,data, delta, reg_sparseness, desired_sparseness, reg_contractive, reg_slowness, data_next):
        ...
        for d in range(data.shape[0]):
            ...
            for i in range(self.input_dim):
                grad_b_i = self._get_gradients(batch,a_h,h,a_y,y,reg_contractive, reg_sparseness, desired_sparseness,
                                                reg_slowness, batch_next, a_h_next, h_next)[1][0,i]
                self.bv[0,i] += delta
                E_pos = self.energy(batch,reg_contractive, reg_sparseness, desired_sparseness, batch_next,reg_slowness)
                self.bv[0,i] -= 2*delta
                E_neg = self.energy(batch,reg_contractive, reg_sparseness, desired_sparseness, batch_next,reg_slowness)
                self.bv[0,i] += delta
                diff_b[d,0,i] = grad_b_i - (E_pos-E_neg)/(2.0*delta)

                for j in range(self.output_dim):
                    grad_c_j = self._get_gradients(batch,a_h,h,a_y,y,reg_contractive, reg_sparseness, desired_sparseness,
